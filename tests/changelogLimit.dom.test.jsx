...
// real one: the module mock below swaps in 14 fake deploys while keeping the REAL visibleEntries
...
// stays a 10-deploy digest, newest first, scrolling within its own region. Everything else about
// the popup lives in changelog.dom.test.jsx against the real data.
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest'
import { render, screen, within, cleanup, fireEvent, act } from '@testing-library/react'

// 14 deploys, newest first: 2026-06-30 down to 2026-06-17, one line each.
...
describe('the Changelog popup caps at the latest 10 deploys (Q6)', () => {
  beforeEach(() => {
    localStorage.clear()
  ...
  it('renders exactly the newest 10 of 14 entries, in order, and none of the older 4', () => {
    ...
    // Newest first, one line per fake deploy: 30 down to 21 shown…
    expect(
      within(dialog)
    ...
    // …and the four older deploys stay repo-only (the module keeps them; the popup is a digest).
